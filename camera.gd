...
func _input(event):
	if event.type == InputEvent.MOUSE_BUTTON:
		if event.button_index == BUTTON_LEFT:
			...
			set_zoom(get_zoom()/1.05)
	if event.type == InputEvent.MOUSE_MOTION && drag:
		var rx = event.pos.y-touch_pos.y
		var ry = -0.004*(event.pos.x-touch_pos.x)
		rotate_camera(rx, ry)
		touch_pos = event.pos
...
func _fixed_process(delta):
	if moving:
		...
		t += 30*delta*(target_pos-t).normalized()
		set_translation(t)
		...
		if translate_x != 0 || translate_y != 0:
			var direction = 10*Vector2(translate_x, translate_y).rotated(rotate_y.get_rotation().y)
